...
$$\sum\limits*{i=1}^{n}p*i(n-i)=\sum\limits*{i=1}^{n}\frac{1}{n}p*i(n-i)=\frac{1}{n}\sum\limits*{i=1}^{n}p_i(n-i)=\frac{1}n\frac{n(n-1)}2=\frac{n-1}2$$
，时间复杂度为 `O(n)`。
...
$$\sum\limits*{i=1}^{n}p*i×i=\sum\limits*{i=1}^{n}\frac{1}{n}i=\frac{1}{n}\sum\limits\_{i=1}^{n}(n-i+1)=\frac{1}{n}\frac{n(n+1)}{2}=\frac{n+1}2$$
，时间复杂度为 `O(n)`。
